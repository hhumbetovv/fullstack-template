...
Utilities shared across development-time packages (scripts, generators, processor) so codegen and CLI workflows stay consistent. Import `package:common_tooling/tooling.dart` when you need access to the helper surface.
...
## Getting started
1. Add `common_tooling` as a workspace dependency (see the root `pubspec.yaml`).
