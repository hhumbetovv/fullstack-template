...
- **Assets (`AppIcons`, `AppImages`)**: Generated by the `gen_assets` builder. `AppIcons` is a font-backed class exposing `IconData` constants (via `icons.dart`) sourced from the generated `assets/fonts/icons.otf`; `AppImages` remains an enum that mirrors the `assets/images` directory with convenience getters. Use these instead of hard-coded paths.
  - SVG ikonları yalnızca `fill` tabanlı path’lerle teslim edin. Stroke tabanlı çizimler font üretiminde bozuluyor; Figma’da “Outline Stroke” uygulayarak veya Inkscape benzeri bir araçla stroke’ları path’e çevirip commitleyin.
...
- Icon SVGs are normalized to snake_case during generation; avoid manually renaming the output files afterwards. Image enum cases still follow the file names (kebab-case on disk, lowerCamelCase in code). Add or rename assets in the folders and rerun the builder instead of editing the generated files manually.
...
- After adding icons or images, run code generation (e.g. `fvm dart run scripts gen-build ui_foundation` or `dart run build_runner build -r`) so the icon font (`assets/fonts/icons.otf`), `AppIcons`, and `AppImages` refresh with the latest files. Ensure the font family entry lives under `flutter.fonts` in `pubspec.yaml` so Flutter can load the generated glyphs.
