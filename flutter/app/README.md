...
4. `App` builds a `MaterialApp` that mounts `NavDisplay` (backed by `AppNavigation`) inside `RootView`.
...
## Router composition
- `navigation/router.dart` exposes `AppNavigation`, which merges feature `NavEntry` lists into a single page stack.
- To expose a new screen:
  1. Export the route name in `core_navigation/lib/src/core/routes.dart` (see core/navigation/README.md).
  2. Add the feature navigation entries to `AppNavigation.entries`.
  3. Ensure the feature presentation package exports its nav keys/entries in `router.dart` (see feature/README.md).
- Deep-link handling still lives in the feature layer; convert resolved links into `NavKey` stacks via the `NavigatorContextExtension` helpers.
...
## Theming
- `MaterialApp` references `BaseTheme.light.data` (see uikit/components/README.md and uikit/foundation/README.md) for consistent styling.
...
3. Import the feature navigation entries in `navigation/router.dart` and append them to `AppNavigation.entries`.
