...
- **Bootstrap**: After installing FVM, run `sh scripts/bootstrap.sh` once. It performs project-wide cleanup, installs pods, fetches packages, runs smart-build, and activates flutterfire CLI.
...
- `fvm dart run scripts gen-build` – run build_runner across modules.
- `fvm dart run scripts smart-build` – dependency-aware incremental builds.
- `fvm dart run scripts build` – produce Android/iOS artifacts (configurable flavours via flags).
- `fvm dart run scripts module-graph` – refresh `build_graph.md` with dependency stats.
- `sh scripts/bootstrap.sh` – runs the bootstrap workflow (FVM setup, Flutter clean/pub get, pod install, smart-build, flutterfire CLI activation).

Artifacts produced by the build command land in `ignores/artifacts/`. Build logs persist under `build_logs/` for troubleshooting. Use `scripts/deeplink.sh <url>` to trigger Android deep-link tests via ADB.
...
- **YAML symlinks**: use `fvm dart run scripts yaml-links` to sync all `pubspec.yaml` and `build.yaml` files into `yaml/` for quick access.

## Useful references
- **Dependency graph**: `build_graph.md` (generated by `module-graph`).
