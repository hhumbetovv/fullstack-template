...
| `smart-build [module?] [--dry-run] [--parallel <n>] [--verbose] [--mode error\|git-change\|all] [--git-changes] [--all]` | Dependency-aware incremental build with mermaid output; `--mode`/`--git-changes`/`--all` select between error-only, git-change, or full builds. |
